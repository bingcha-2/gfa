...
export function StatusLookupForm({ compact = false, kind = "order", buttonLabel, onLookup }: StatusLookupFormProps) {
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (onLookup) {
      startTransition(() => { void onLookup(normalized); });
    }
  }

  const fieldId = compact ? `status-lookup-${kind}-compact` : `status-lookup-${kind}`;
  const label = kind === "code" ? "输入卡密" : "输入订单号";
  const placeholder = kind === "code" ? "例如 JZ...，或者 HH / CX 开头卡密" : "例如 GFA-MA3L7Q-9K2W";
  const helperText = kind === "code"
  ...
  return (
    <form className="field-grid" onSubmit={onSubmit}>
      <div className="field">
        ...
        <input id={fieldId} className="mono" placeholder={placeholder} required
          value={value} onChange={(e) => setValue(e.target.value.toUpperCase())} />
        <small>{helperText}</small>
      </div>
      <div className="field-actions">
        <button className="button" disabled={isPending} type="submit">
          {isPending ? "查询中..." : buttonLabel ?? (kind === "code" ? "按卡密查询进度" : "查询订单状态")}
        </button>
      </div>
    </form>
  );
