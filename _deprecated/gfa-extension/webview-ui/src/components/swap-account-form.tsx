...
export type SwapSuccessPayload = SwapResponse & { swapCode: string; newEmail: string };
type SwapAccountFormProps = { onSuccess?: (payload: SwapSuccessPayload) => void };
    ...
    startTransition(async () => {
      try {
        const data = await apiRequest<SwapResponse>("public/swap-by-email", {
          method: "POST",
          body: { swapCode: normalizedCode, originalEmail: normalizedOriginalEmail, newEmail: normalizedNewEmail },
        });
        setResult(data);
        onSuccess?.({ ...data, swapCode: normalizedCode, newEmail: normalizedNewEmail });
      } catch (err) {
        setResult(null);
      ...
      <div className="panel-stack">
        <form className="field-grid" onSubmit={onSubmit} style={{ marginTop: '16px' }}>
          <div className="field">
            ...
            <input id="swap-code" autoComplete="off" className="mono" placeholder="例如 HH1234... 或 CX5678..."
              required value={swapCode} onChange={(e) => setSwapCode(e.target.value.toUpperCase())} />
            <small>换号卡密是专属类型，普通邀请卡密无法使用此功能。</small>
          </div>
          <div className="field">
            <label htmlFor="swap-original-email">原账号邮箱</label>
            <input id="swap-original-email" autoComplete="email" inputMode="email" placeholder="your-original-account@gmail.com"
              required type="email" value={originalEmail} onChange={(e) => setOriginalEmail(e.target.value.trimStart())} />
            <small>之前兑换进组时使用的 Google 账号邮箱。</small>
          </div>
          <div className="field">
            <label htmlFor="swap-new-email">新 Google 邮箱</label>
            <input id="swap-new-email" autoComplete="email" inputMode="email" placeholder="newaddress@gmail.com"
              required type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value.trimStart())} />
