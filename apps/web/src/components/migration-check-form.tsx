...
export function MigrationCheckForm() {
  ...
  function startPolling(currentEmail: string) {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const data = await apiRequest<CheckMigrationResponse>("public/check-migration", {
          method: "POST",
          body: { email: emailRef.current },
        });

        setCheckResult(data);

        // Stop polling once sync is done (no longer needsSync)
        if (!data.needsSync) {
          stopPolling();
        }
      } catch {
        // Silently retry — don't stop polling on transient errors
      }
    }, 3000);
  }
  ...
  function onCheck(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        // If sync triggered, start polling
        if (data.needsSync) {
  ...
  async function onMigrate() {
    ...
    try {
      const data = await apiRequest<SelfMigrateResponse>("public/self-migrate", {
        method: "POST",
        body: { email: normalizedEmail },
      });

      setMigrateResult(data);
      // Clear the check result once migration is done to avoid stale UI
      if (data.success) {
        setCheckResult(null);
      }
    } catch (err) {
    ...
    <section className="form-card">
      <div className="panel-stack">
        ...
        {showMemberInfo && checkResult.memberInfo ? (() => {
          ...
          return (
            <div className="notice" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px', background: checkResult.needsMigration ? 'rgba(239, 68, 68, 0.1)' : 'rgba(234, 88, 12, 0.1)', border: `1px solid ${checkResult.needsMigration ? 'rgba(239, 68, 68, 0.3)' : 'rgba(234, 88, 12, 0.3)'}` }}>
        ...
        {/* Non-eligible result (no memberInfo) */}
        {checkResult && !checkResult.eligible && !checkResult.memberInfo && !migrateResult ? (
          <div className="notice warn" style={{ marginTop: '16px' }}>
            {checkResult.message}
          </div>
        ) : null}
        ...
        {migrateResult ? (
          <div className="notice" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px', background: migrateResult.success ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)', border: `1px solid ${migrateResult.success ? 'rgba(34, 197, 94, 0.3)' : 'rgba(239, 68, 68, 0.3)'}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
