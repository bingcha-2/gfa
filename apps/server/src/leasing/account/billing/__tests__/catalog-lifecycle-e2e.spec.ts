 ...
 *
 * 容量口径:test env BCAI_ACCOUNT_SHARE_CAPACITY=4(vitest.config.ts);共享人数 N → weight=8/N,
 * 但份额封顶按 4 算。下面所有座位算术都 import ACCOUNT_SHARE_CAPACITY,绝不硬编码。
 */
...
describe("E2E 绑定线:下单 → 激活 → 分配座位(写 config.bindings)→ 钉号租 → fair-share", () => {
  it("整条链跑通:config.line=bind、座位钉到 fixture 号、requiresBinding、租到钉的号", async () => {
    ...
    // 绑定线:anthropic max-20x,1 人独号(weight=8,但容量 4)。
  ...
  it("绑定线 2 人拼车:weight=8/2=4、占该号 4 份(=容量)、第二张同号订阅无座位", async () => {
    await publishCatalog();
    const c1 = await createTestCustomer();
    const c2 = await createTestCustomer();

    // 第一单 2 人拼车 → weight 4,占满 account 1(容量 4)。
    ...
    expect(cfg(first.sub).weight).toBe(8 / 2);
    expect(cfg(first.sub).bindings).toEqual({ anthropic: 1 });

    // 第二单同等级同 weight:容量已被第一单占满(4/4)→ 下单座位预检直接 BadRequest。
    await expect(billingService.createCatalogOrder(c2.id, sel, "ALIPAY")).rejects.toThrow(/暂无可用座位|BadRequest/i);
  });
});
...
describe("E2E 号池 vs 绑定:座位/调度差异", () => {
  it("号池不占座位(绑定线仍能拿满该号份额);绑定占座位(动态池不调度出池号)", async () => {
    ...
    // 绑定线独号(weight=8→clamp 容量 4)仍能拿到 account 1 的全部份额 —— 证明号池没占座位。
...
describe("E2E 取消/退款/过期:座位释放复用、过期租号拒", () => {
  it("退款(取消订阅)→ 座位释放,后续同号订阅可复用该座位", async () => {
    ...
    await expect(billingService.createCatalogOrder(c2.id, sel, "ALIPAY")).rejects.toThrow(/暂无可用座位/);
...
describe("E2E 边界/异常", () => {
  it("绑定座位满(下单时该等级无空闲号)→ 下单 BadRequest(预检挡在付款前)", async () => {
    await publishCatalog();
    // 用满 account 1(容量 4):一个 weight-4(2 人拼车)订阅占满。
    const cFull = await createTestCustomer();
    await purchaseAndActivate(cFull.id, { line: "bind", items: [{ product: "anthropic", level: "max-20x" }], shareUsers: 2, deviceLimit: 1 });
