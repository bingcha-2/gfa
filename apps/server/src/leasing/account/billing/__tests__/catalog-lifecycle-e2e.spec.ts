...
describe("E2E 号池线:配 catalog → 发布 → GET → 下单 → 回调激活 → 租号 → 上报 → 再租", () => {
  it("整条链跑通:config.line=pool、无座位、动态池选号、用量计回订阅、可重复租", async () => {
    ...
    expect(store.publicStatus(store.findById(sub!.id)!).recentWindowTokens).toBe(150);
...
describe("E2E 绑定线:下单 → 激活 → 分配座位(写 config.bindings)→ 钉号租 → fair-share", () => {
  it("整条链跑通:config.line=bind、座位钉到 fixture 号、requiresBinding、租到钉的号", async () => {
    ...
    // 绑定线:anthropic max-20x,1 人独号(weight=cap=4,占用 4 个权益席位;销售容量默认 10)。
  ...
  it("绑定线默认销售容量 10:2 人拼车可售 5 张、第 6 张同号订阅无座位", async () => {
    ...
    // 5 张各占 2 席,合计占满默认销售容量 10;第 6 张预检直接 BadRequest。
    await expect(billingService.createCatalogOrder(customers[5].id, sel as any, "ALIPAY")).rejects.toThrow(
      /暂无可用座位|BadRequest/i,
    );
  });
});
...
describe("E2E 号池 vs 绑定:座位/调度差异", () => {
  it("号池不占座位(绑定线仍能拿满该号份额);绑定占座位(动态池不调度出池号)", async () => {
    ...
    // 绑定线独号(weight=cap=4)仍能绑定到 account 1 并占用自己的权益席位 —— 证明号池没占座位。
    ...
    expect(occupiedBind).toBe(ACCOUNT_SHARE_CAPACITY); // 独号占用 4 个权益席位
    const occupiedPool = configs.filter((x: any) => x.line === "pool").length;
    expect(occupiedPool).toBe(1); // 存在号池订阅,但它不占座位(上面 bind 仍能绑定即证)
  });
});
