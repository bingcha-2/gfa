 ...
 * the device management endpoints (global prefix "api" + controller "account/devices"):
 *
 *   GET   /api/account/devices            → {devices, deviceLimit}
 *   PATCH /api/account/devices/:id        → {ok:true, device}
 *   POST  /api/account/devices/:id/revoke → {ok:true}
 */
...
describe("DeviceController metadata", () => {
  it("mounts at account/devices with GET /, PATCH :id, POST :id/revoke", () => {
    expect(Reflect.getMetadata("path", DeviceController)).toBe("account/devices");
