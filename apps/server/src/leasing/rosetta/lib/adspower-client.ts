/**
 * AdsPower Local API client for apps/server
 *
 ...
 * Supports setting user_proxy_config dynamically on start.
 */
...
export class AdsPowerClient {
  ...
  async openProfile(
    ...
    const result = new Promise<OpenProfileResult>((resolve, reject) => {
      this._openMutex = this._openMutex.then(async () => {
        try {
          const r = await this._openProfileImpl(profileId, userProxyConfig, canForceClose);
          resolve(r);
        } catch (e) {
          reject(e);
        }
  ...
  private async _openProfileImpl(
    ...
    const params: Record<string, string> = { user_id: profileId };
    if (userProxyConfig) {
      params.user_proxy_config = JSON.stringify(userProxyConfig);
    }
    const url = this.buildUrl("/api/v1/browser/start", params);
