...
describe("getAccountBanAnalysis — 每客户峰值/分 + 来源IP + 持卡数(点开母号看哪个买家)", () => {
  it("按客户从同一次 RequestLog 扫描聚合峰值 req/min、不同来源 IP、持卡数", async () => {
    ...
    expect(cu.cliReqs).toBe(3);              // 三条都是 cli
    expect(cu.desktopReqs).toBe(0);
  });

  it("接管面按客户分 cli/desktop;customerId 经 Customer 表换成邮箱", async () => {
    const rows = [
      { accountEmail: "a@x.com", accessKeyId: "k1", customerId: "cust9", bucket: "anthropic-claude", requests: 3, failedRequests: 0, reverseProxyHits: 0, totalTokens: 10 },
    ];
    const logRows = [
      { provider: "anthropic", accountEmail: "a@x.com", accessKeyId: "k1", customerId: "cust9", surface: "cli", sourceIp: "1.1.1.1", exitIp: "", userId: "u1", at: new Date("2026-06-23T00:00:05Z") },
      { provider: "anthropic", accountEmail: "a@x.com", accessKeyId: "k1", customerId: "cust9", surface: "desktop", sourceIp: "1.1.1.1", exitIp: "", userId: "u1", at: new Date("2026-06-23T00:00:40Z") },
    ];
    const prisma = {
      cardUsageHourly: { findMany: vi.fn().mockResolvedValue(rows) },
      requestLog: { findMany: vi.fn().mockResolvedValue(logRows) },
      customer: { findMany: vi.fn().mockResolvedValue([{ id: "cust9", email: "buyer@mail.com" }]) },
    };
    const res = await makeService(prisma).getAccountBanAnalysis({ days: 7 });
    const cu = res.accounts[0].customers[0];
    expect(cu.customerEmail).toBe("buyer@mail.com");
    expect(cu.cliReqs).toBe(1);
    expect(cu.desktopReqs).toBe(1);
  });
});
...
describe("getBanComparison — 已封 vs 健康 定因对比", () => {
  it("两组均值 + 差异倍数,按差异降序,峰值 req/min 从 RequestLog 时间戳算", async () => {
    ...
    // 桌面端占比分母 = 已上报 surface 的请求(cli+desktop+ide),不含空 surface。
    // a@x: desktop 1 / (desktop1+cli2)=3 → 1/3;b@x: 0/1 → 0。
    const dt = res.metrics.find((m) => m.key === "desktopRatio")!;
    expect(dt.bannedAvg).toBeCloseTo(1 / 3);
