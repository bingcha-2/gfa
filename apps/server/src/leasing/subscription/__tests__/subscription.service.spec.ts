...
describe("SubscriptionService.createFromCatalog / activateForOrder (catalog 下单激活)", () => {
  ...
  it("forceNew:同配置也一律新建独立订阅(激活码语义,绕过续期叠加)", async () => {
    const customer = await createTestCustomer();
    await publishCatalog(30, 1);
    const poolConfig = {
      line: "pool",
    ...
    const first = await service.activateForOrder(mkOrder("code-order-1"), { forceNew: true });
    // 第二单 config 完全相同,但 forceNew → 不续期、新建并存。
    const second = await service.activateForOrder(mkOrder("code-order-2"), { forceNew: true });
