 ...
 *   GET  /api/web/tickets              list tickets (newest first)
 *   POST /api/web/tickets              create ticket (OPEN + first CUSTOMER message)
 *   GET  /api/web/tickets/:id          detail + messages (ownership)
 *   POST /api/web/tickets/:id/messages reply (ownership; 409 if CLOSED; re-open if ANSWERED)
 */
@Controller("web/tickets")
@Public()
...
export class TicketController {
  ...
  /**
   * POST /api/web/tickets/:id/messages
   * Body: { body(≤4000) }
