...
CREATE TABLE IF NOT EXISTS local_accounts (
  ...
  expiry INTEGER, is_gcp_tos INTEGER, sort_order INTEGER, quota_buckets TEXT
);`
...
const allCols = `id,provider,email,auth_kind,id_token,access_token,refresh_token,api_key,api_base_url,
  ...
  expiry,is_gcp_tos,sort_order,quota_buckets`
...
func (s *Store) Add(a *Account) error {
	...
	_, err := s.db.Exec(`INSERT INTO local_accounts (`+allCols+`)
	  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		...
		a.Expiry, b2i(a.IsGCPTos), a.SortOrder, buckets)
	return err
}
...
func (s *Store) Update(a *Account) error {
	...
	_, err := s.db.Exec(`UPDATE local_accounts SET email=?,auth_kind=?,id_token=?,access_token=?,refresh_token=?,
	  ...
	  last_used_at=?,updated_at=?,project_id=?,name=?,expiry=?,is_gcp_tos=?,sort_order=?,quota_buckets=? WHERE id=?`,
		...
		a.Expiry, b2i(a.IsGCPTos), a.SortOrder, marshalBuckets(a.Buckets), a.ID)
	return err
}
...
func scan(rows *sql.Rows) ([]*Account, error) {
	...
	for rows.Next() {
		...
		var tags string
		var pool, prio int
		var name, buckets sql.NullString
		...
		if err := rows.Scan(&a.ID, &a.Provider, &a.Email, &a.AuthKind, &a.IDToken, &a.AccessToken, &a.RefreshToken,
			...
			&a.CreatedAt, &a.LastUsedAt, &a.UpdatedAt, &a.ProjectID, &name, &expiry, &gcp, &sortOrder, &buckets); err != nil {
			return nil, err
		}
		a.Name = name.String
