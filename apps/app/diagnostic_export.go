...
// ExportDiagnosticBundle lets the user save a local-only, redacted support ZIP.
...
func (a *App) ExportDiagnosticBundle() (string, error) {
	...
	path, err := wailsruntime.SaveFileDialog(a.ctx, wailsruntime.SaveDialogOptions{
		Title:                "导出诊断包",
		DefaultFilename:      "BingchaAI-diagnostics-" + now.Format("20060102-150405") + ".zip",
	...
	path, err = normalizeDiagnosticPath(path)
	if err != nil {
		return "", err
	}
