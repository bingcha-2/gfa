...
func TestDiagnosticOutputPathIncludesMilliseconds(t *testing.T) {
	first := diagnosticFallbackOutputPath(time.Date(2026, time.September, 1, 12, 0, 0, 100_000_000, time.Local))
	second := diagnosticFallbackOutputPath(time.Date(2026, time.September, 1, 12, 0, 0, 200_000_000, time.Local))
	if first == second {
		t.Fatalf("diagnostic output paths collide: %q", first)
	}
}
