...
func (m *mitmManager) buildHandler() http.Handler {
	...
	// claude.ai 出口:走用户网络(upstream/系统代理),透传用户自己的 claude.ai 登录态。
	// 借号(白号 sessionKey 注入)已移除 —— claude.ai 始终以用户自己的账号登录,MITM 只做订阅等级/Max 改写。
	claudeAiProxyFn := func() string { return m.userProxy() }
	claudeAi := mitmClaudeAiHandler(newClaudeUpstreamTransportFn(claudeAiProxyFn))
