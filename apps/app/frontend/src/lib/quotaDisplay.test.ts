...
import {
  ...
  formatResetDuration,
  isExclusiveCard,
  mergeRuntimeQuotaIntoSubscriptions,
  monotonicQuotaValue,
  nestedBarDisplay,
...
describe('mergeRuntimeQuotaIntoSubscriptions', () => {
  it('uses runtime Anthropic split quota without overwriting personal fair-share quota', () => {
    const got = mergeRuntimeQuotaIntoSubscriptions(
      [
        {
          id: 'sub-anthropic',
          status: 'ACTIVE',
      ...
      ],
      {
        claudeQuota: {
          ...
          weeklyFraction: 0.36,
          hourlyResetMs: 0,
          weeklyResetMs: 4 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000,
        },
        now: Date.UTC(2026, 6, 9, 8, 0, 0),
      },
    )

    expect(got[0].productQuota?.anthropic).toMatchObject({
      hourlyPercent: 99,
      weeklyPercent: 36,
      myHourlyFraction: 1,
      myWeeklyFraction: 1,
    ...
    expect(got[0].productQuota?.anthropic.weeklyResetAt).toBe('2026-07-13T10:00:00.000Z')
  })

  it('does not overwrite personal quota when a subscription is display-exclusive', () => {
    const got = mergeRuntimeQuotaIntoSubscriptions(
      [
        {
          id: 'sub-display-exclusive',
          status: 'ACTIVE',
          expiresAt: '',
          deviceLimit: 1,
          priority: 0,
  ...
  it('keeps full-share personal weekly quota even when the account weekly quota is low', () => {
    const got = mergeRuntimeQuotaIntoSubscriptions(
      [
        {
          id: 'cmr35g2lq07nts1ica9574mok',
          status: 'ACTIVE',
          expiresAt: '',
          deviceLimit: 1,
          priority: 0,
...
describe('nestedBarDisplay (客户端遮超卖:8人车口径 + 账号封顶)', () => {
  ...
  it('N5 独享卡(单层):myTotalRemain = myFraction(不看 accountFraction,不按名义份额缩放)', () => {
    const d = nestedBarDisplay({ myFraction: 0.3, accountFraction: 0.05, shareSeats: 8, shareCapacity: 8, exclusive: true })
    expect(d.myTotalRemain).toBeCloseTo(0.3, 6) // 直接 = myFraction,不被 accountFraction=0.05 封顶
    expect(d.accountRemain).toBe(-1) // 独享不暴露账号层
  })

  it('N5b 独享卡:accountFraction 更低也不影响 myTotalRemain', () => {
    const d = nestedBarDisplay({ myFraction: 0.8, accountFraction: 0.1, shareSeats: 1, shareCapacity: 8, exclusive: true })
    expect(d.myTotalRemain).toBeCloseTo(0.8, 6) // 拼车会被封顶到 0.1,独享不会
  })
