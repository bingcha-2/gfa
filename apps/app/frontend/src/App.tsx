...
const SERVER_HEARTBEAT_INTERVAL_MS = 20 * 60 * 1000
...
export default function App() {
  ...
  // Polling: server heartbeat every ~20min while logged in(usePolling 串行链 +
  // store 在途守护 → 永不重叠)。会话吊销/订阅到期由 Go 侧落地;心跳完成后刷新
  // 账号态 —— SESSION_INVALID/DEVICE_REVOKED → 登录页,SUBSCRIPTION_EXPIRED → 横幅。
  // 登录态刚建立时仍立即执行一次，之后才按 20min 间隔调度。
  usePolling(heartbeat, SERVER_HEARTBEAT_INTERVAL_MS, isLoggedIn)

  // Polling: stats every 2s, IDE every 15s, announcement every 30min (only when logged in)
