...
桌面客户端连接账号代理时，先使用显式配置的本地代理；未显式配置时读取系统代理。因此链路可以是「客户端 → Clash/Mihomo 系统代理 → 账号代理 → 上游」，无需强制使用 TUN。本地代理规则必须允许访问账号代理的地址和端口；未配置本地/系统代理时直接连接账号代理。系统代理沿用启动后首次读取并缓存的机制，修改系统代理设置后请重启桌面客户端。HTTP、WebSocket 和接管预检使用相同链路；启用指纹时任一跳失败均不会绕过账号代理。此链路修复只需更新桌面客户端，服务端无需配套改动。
