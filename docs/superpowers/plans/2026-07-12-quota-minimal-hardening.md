...
- [x] **Step 2: Run focused spec, confirm RED** (missing method / no 503).
- [x] **Step 3: Implement** readiness flag + retry loop + gate + ready-callback.
- [x] **Step 4: Run focused server specs GREEN**; run full `pnpm --dir apps/server exec vitest run src/leasing/` GREEN.
- [x] **Step 5: Commit** `fix(quota): gate membership reconciliation on subscription readiness`
...
- [x] **Step 1: Write failing spec** — apply a usage report without awaiting `checkpointReport`; trigger `flush()`; assert the repository `checkpointBatch` payload contains the reportId + accounting. Second case: after a successful commit, a later flush does not resend the same reportId.
- [x] **Step 2: Run focused spec, confirm RED** (payload has empty reportIds).
- [x] **Step 3: Implement** pending-receipt bookkeeping (populate, attach on both flush paths, clear on commit success, retain on failure).
- [x] **Step 4: Run tracker + repository + lease-service specs GREEN.**
- [x] **Step 5: Commit** `fix(quota): persist pending receipts on scheduled flush`
...
- [x] **Step 1: Write failing Go test** — seed `recordMyPersonalBucketFraction("codex-gpt", 0.9)`, run the logout state-clearing path, assert `snapshotMyPersonalFractions()` is empty; same for the login path.
- [x] **Step 2: Run `go test -run TestLogout -count=1 .`, confirm RED.**
- [x] **Step 3: Implement** the three call sites.
- [x] **Step 4: Run `go test ./... -count=1` GREEN.**
- [x] **Step 5: Commit** `fix(app): clear quota blood bars on logout and login`
...
- [x] **Step 2: Run focused spec, confirm RED** (lease already swept at 20 min).
- [x] **Step 3: Implement** — sweep threshold uses `REPORT_REPLAY_GRACE_MS`; delete the now-unused `REPORT_GRACE_MS` or repoint it.
- [x] **Step 4: Run lease-service + full server specs GREEN.**
- [x] **Step 5: Commit** `fix(lease): retain expired leases through the 35-minute replay grace`
...
- [x] **Step 1: Add the scenarios and clock reordering; run `pnpm test:quota-e2e`, confirm the new scenarios RED against pre-fix behavior only where the fix is not yet merged (if executing after Tasks 1–4, they must be GREEN; verify each scenario actually exercises its fix by temporarily reverting the fix commit locally if in doubt).**
- [x] **Step 2: Run the full matrix 5 consecutive times: `for i in 1 2 3 4 5; do node tests/quota-e2e/run.mjs || break; done` — 5/5 GREEN.**
- [x] **Step 3: Commit** `test(quota): isolate e2e clock and cover hardening fixes end to end`

### Task 6: Regression gate

- [x] Run `pnpm --dir apps/server test` — all pass.
- [x] Run `cd apps/app && go test ./... -count=1` — all pass.
- [x] Run `pnpm --dir apps/app/frontend test` — all pass.
- [x] Run `pnpm test:quota-e2e` ×3 — all pass.
- [x] Commit any doc updates; do not push without an explicit ask.
