...
**关键:** 实现 CLIProxyAPI 的 `coreauth.Store` 接口,其 `Load()` **只从本地 account.Store 读 PoolEnabled 自有号**,转成 `*coreauth.Auth`。网关账号唯一入口是这个 Store → lease 没有进入路径(编译期保证:lease 类型不经过此 store)。
...
func TestStore_LoadOnlyOwnPoolAccounts(t *testing.T) {
	...
	auths, err := st.Load(context.Background())
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if len(auths) != 1 {
...
func (s *Store) Load(ctx context.Context) (map[string]*coreauth.Auth, error) {
	list, err := s.acc.ListPoolEnabled(s.provider)
	if err != nil {
		return nil, err
	}
	out := make(map[string]*coreauth.Auth, len(list))
	for _, a := range list {
		out[a.ID] = toAuth(a)
	}
	return out, nil
...
// Save/Delete/SetBaseDir 满足接口;Save 不持久化(单一事实源在 account.Store)。
