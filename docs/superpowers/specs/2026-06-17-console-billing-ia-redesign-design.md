...
- **测试驱动(TDD)**:每块先写失败测试,再写实现使其通过。块 1/2(订阅详情 + 换绑)、GRANT 语义、席位释放是重点测试对象(见"测试要点")。
- **删废旧代码**:改到的文件顺手清理被本次取代的死代码(例如旧 `RebindDialog` 被订阅详情逐产品换绑取代后应删除,而非留存)。不做无关重构。
- **单文件 ≤ 800–1000 行**:新建/改动后超出即拆分(订阅详情枢纽、客户一屏全貌这类大组件优先按子区块拆成独立、可单测的小组件)。客户详情页、usage-stats 等本就偏大的文件,改动时一并向此约束靠拢。
