...
| 数据 | 保留 | 硬上限 | 说明 |
|---|---:|---:|---|
| `RequestLog` | 48h | 500,000 行 | 每天 04:00 小批清理；header 上限 2 KB |
| `QuotaDiagnosticEvent` | 72h | 150,000 行 | 状态变化事件；单个 JSON 字段 ≤4 KB |
| `QuotaReportReceipt` | 24h | — | 每天 03:00 清理；覆盖客户端重试周期 |
| `AccountQuotaSnapshot` | 7d | — | 每天 02:00 清理；额度水位历史，不参与计费 |
| `TaskLog` | 24h | — | 只清明确终态任务的日志，每天 03:30 清理 |
| `Task` | 15d | — | 只清明确终态；保留可重试和待人工处理任务 |
| 客户端/服务端 quota 结构化文件日志 | 72h | 单文件 100 MB、总量 500 MB | 按年龄与总量双重轮转，旧文件压缩 |

清理规则：

- 数据库清理统一安排在北京时间凌晨，每天执行一次，并错开各表时间。
- 按各表 TTL 分批删除，每批最多 500 行；每批提交后重新进入低优先级队列，先让出写入机会给 checkpoint 和业务写入。
...
- 任一线上问题能在 48 小时内按 trace/report/lease/account/subject 查询完整决策链并导出脱敏支持包。
- 逐请求日志保留 48 小时，额度水位保留 7 天；回执和终态任务日志保留 24 小时。
