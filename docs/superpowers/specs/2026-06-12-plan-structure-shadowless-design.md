...
- **上线不兼容老卡密**:不再保留"老卡密走 `access-keys.json` 文件影子"那条路。老卡密只能经 `bind-card` 迁移成订阅才能用;上线后 `access-keys.json` 连带 `reload` / 文件 cache / `byId` 整个退役删除。**这一刀砍掉了文件↔内存双表、reload 协同、座位从文件数 等全部复杂性。**
...
号池 vs 绑定:**只看 `config.line`(显式 mode)** —— 下单时写定,激活 / 限额 / 座位全程读它,**不再靠 `bindings` 空不空隐式推断**(隐式推断在"激活前 bindings 必空"时分不清号池/绑定,且到处 `boundAccountId===0` 难维护)。
...
## 6. 落地架构:订阅唯一真相源(无老卡密版)

> **★结构塌缩★**:因上线不兼容老卡密(§2),`access-keys.json` / 文件 cache / `reload` 全部退役。**订阅(数据库)是唯一真相源,内存只是它的缓存 + 用量计数。**
>
> - **配置**:运行时从订阅(内存缓存,源是 DB)读,无文件影子。
> - **号池 vs 绑定**:读 `config.line`(显式),不推断。
> - **座位占用**:`数据库 count`(`line=bind` 且绑某号的订阅数 < 该号容量),不再从文件数。
> - **用量**:内存窗口 + `CardTokenUsage`(本就不在文件)。
>
> 下面 6.1–6.5 中凡涉及"文件影子并存 / 两路查 / reload 协同 / 老卡密并存"的描述均作废 —— 没有文件,这些都不存在。`access-key-store` 从"文件缓存"降级为"订阅 record 缓存 + 用量"。
