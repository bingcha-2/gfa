...
GFA/
├── apps/
│   ├── app/             # 冰茶AI 桌面端 (Go + Wails, was bcai-wails)
│   ├── server/          # 业务 API + Token 租赁服务 (NestJS, was api)
│   ├── web/             # 官网 + 用户中心 (account) + 管理后台 (console) (Next.js)
...
| 应用 | 技术栈 | 说明 |
|---|---|---|
| [`app`](apps/app) | Go · Wails · WebView | 桌面客户端,内嵌代理引擎,监听 `127.0.0.1:60670` 并转发请求到 Google API |

### 🖥️ 服务端

| 应用 | 技术栈 | 说明 |
|---|---|---|
| [`server`](apps/server) | NestJS · Prisma · BullMQ | 业务 API + Token 租赁 + Rosetta 管理 (`/api/web/*` · `/api/console/*` · `/api/app/*`) |
...
- [命名规范](docs/NAMING.md) — 权威命名表：目录、域、HTTP 接口、路由、Cookie
...
| 目录 | 原用途 | 替代方案 |
|---|---|---|
| `bcai-client/` | Electron 桌面端 + 独立 Go 代理引擎 | → `apps/app/`（Wails 一体化） |
| `gfa-client/` | Tauri 桌面端 (v3.x) | → `apps/app/`（Wails 一体化） |
